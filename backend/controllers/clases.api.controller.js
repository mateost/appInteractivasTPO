...
export function findTipo(req, res) {
  ...
  service.findTipo(tipo).then(function (clases) {
    if (clases) {
      res.status(200).json(clases);
    } else {
      res
        .status(404)
        .json({ message: `La clase #${id} no se encuentra en el sistema.` }); // ok
    }
  });
}
...
export function findMateria(req, res) {
  ...
  service.findMateria(materia).then(function (img) {
    if (img) {
...
export function findMaterias(req, res) {
  service.findMaterias().then(function (materias) {
...
export function findOne(req, res) {
  ...
  service.findOne(id).then(function (clase) {
    if (clase) {
...
export function create(req, res) {
  ...
  service.create(clase).then(function (clase) {
...
export function editOne(req, res) {
  ...
  service.updateClase(id, claseData).then(function (clase) {
    if (clase) {
      res.status(200).json(clase);
    } else {
      res
        .status(404)
        .json({ message: `La clase no se encuentra en el sistema.` }); // ok
    }
  });
}
...
export function changeOne(req, res) {
  ...
  service.changeOne(id, claseData).then(function (clase) {
    if (clase) {
      res.status(200).json(clase);
    } else {
      res
...
export function removeOne(req, res) {
  ...
  service.removeOne(id).then(function (clases) {
...
export function findProfesor(req, res) {
  ...
  service.findProfesor(id).then(function (clase) {
    if (clase) {
      res.status(200).json(clase);
    } else {
      res
