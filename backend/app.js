...
import FiguraRoute from './routes/figuras.route.js'
import PeliculaRoute from './routes/peliculas.route.js'
import PersonajeRoute from './routes/personajes.route.js'
...
const socketClient = serverSocket.on('connection', (socket) => { //se lo manda a todos
...
app.all('*', (req, res, next) => { //creando un middleware
...
var storage = multer.diskStorage({ //define el donde y como
  destination: function (req, file, cb) { //cb parametros de la function de destination
...
app.post('/api/upload/:seccion', upload.single('NOMBRE'),(req, res) => { //seccion se usa para definir el directorio
