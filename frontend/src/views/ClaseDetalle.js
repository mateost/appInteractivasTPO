...
import { CommentOne, findOne } from "services/clases.service.js";
...
export default function Profile({
    ...
    const alumnoId = localStorage.getItem("_id");
    const { id } = useParams();
    const test = async () => await findOne(id);
    useEffect(() => {
    ...
    const onSubmit = (e) => {
        console.log(
            JSON.stringify({ alumno: alumnoId, comentario: comentario })
        );
        CommentOne(id, alumnoId, comentario)
            .then((clase) => {
                console.log(clase);
            })
            .catch((errorMsg) => {
