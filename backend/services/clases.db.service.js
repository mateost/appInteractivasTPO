...
export async function findMateria(materia) {
  return cliente
    ...
    .then(async function () {
      ...
      const img = await db
        .collection("materias")
        .findOne({ nombre: materia }, { projection: { img: 1, _id: 0 } });
      return img.img;
    })
    .catch(() => {});
}
export async function findMaterias() {
  return cliente.connect().then(async function () {
    ...
    const materias = await db
      .collection("materias")
      .find({}, { projection: { nombre: 1, img: 1, _id: 0 } })
      .toArray();
    return materias;
  });
}
...
export async function create(clase) {
  return cliente.connect().then(async function () {
    ...
    await db
      .collection("clases")
      .updateOne(
        { _id: new ObjectId(clase._id) },
        { $set: { profesorId: ObjectId(clase.profesorId) } }
      );
    return clase;
  });
}
...
export async function updateClase(id, clase) {
  ...
  return cliente.connect().then(async function () {
    ...
    await db
      .collection("clases")
      .updateOne({ _id: new ObjectId(id) }, { $set: clase });
    return clase;
  });
}
...
export async function search(filtros) {
  ...
  const db = cliente.db("trk");
  const clases = await db.collection("clases").findMany(filtros);
}
